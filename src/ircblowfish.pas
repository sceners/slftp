...
procedure irc_RegisterChannel(netname, channel, blowkey: string; chankey: string = ''; inviteonly: Boolean= False);
...
begin
  ...
  begin
    ...
    if ((bf.channel = channel) and (bf.netname = netname)) then
    begin
      Result:= bf;
      exit;
    end;
...
procedure irc_RegisterChannel(netname, channel, blowkey: string; chankey: string = ''; inviteonly: Boolean= False);
begin
  if nil = FindIrcBlowfish(netname, channel, False) then
    chankeys.Add(TIrcBlowkey.Create(netname, channel, blowkey, chankey, inviteonly));
end;

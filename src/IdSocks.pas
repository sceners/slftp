...
type
  ...
  public
    ...
    procedure MakeSocksConnection(const AHost: string; const APort: Integer; const ATimeout: Integer);
    procedure MakeSocks4Connection(const AHost: string; const APort: Integer);
    procedure MakeSocks5Connection(const AHost: string; const APort: Integer; const ATimeout: Integer);
    //
...
procedure TIdSocksInfo.MakeSocks5Connection(const AHost: string; const APort: Integer; const ATimeout: Integer);
var
  len, pos: Integer;
...
begin
  ...
  try
    if FIOHandler.Readable(ATimeout) then
      FIOHandler.Recv(tempBuffer, 5)    // Socks server replies on connect, this is the first part
    else
      raise Exception.Create('Timeout on connecting via socks5');
  except
    raise EIdSocksServerRespondError.Create(RSSocksServerRespondError);
  end;
...
procedure TIdSocksInfo.MakeSocksConnection(const AHost: string; const APort: Integer; const ATimeout: Integer);
begin
  case Version of
    svSocks4, svSocks4A: MakeSocks4Connection(AHost, APort);
    svSocks5: MakeSocks5Connection(AHost, APort, ATimeout);
  end;
end;
